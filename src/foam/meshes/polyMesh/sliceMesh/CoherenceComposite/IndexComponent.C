...
Foam::IndexComponent::IndexComponent( const Foam::string type,
    ...
    , init_strategy_{ Foam::create_component_initializer( type, name, !calc_start_ ) }
    , initialized_{ false } {}
...
void Foam::IndexComponent::init() {
    ...
    Foam::InitStrategy::labelPair start_count( parent->accept( calc_start_ ),
...
void Foam::IndexComponent::init_children() {
    using std::begin;
    using std::end;
    std::for_each( begin( components_map_ ),
                   end( components_map_ ),
...
void Foam::IndexComponent::_v_pull_node_( const Foam::string& by_name, base_ptr& output ) {
    if ( output ) { return; }
...
void Foam::IndexComponent::_v_extract_( Foam::DataComponent::index_container& output ) {
    output = data_;
}
